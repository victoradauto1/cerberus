...
describe('AutomationController tests', () => {
  ...
  beforeAll(async () => {
    ...
    automationController =
      moduleFixture.get<AutomationController>(AutomationController);
  });
  ...
  it('should get automations', async () => {
    const automations =
      await automationController.getAutomations(authorization);
  ...
  it('should NOT add automation (price1)', async () => {
    userServiceMock.useValue.getUser = jest
      .fn()
      .mockResolvedValue({ ...activeAutomationMock, privateKey: null });
    const automationData = { ...activeAutomationMock } as AutomationDTO;
    await expect(
  ...
  it('should start automation (price1)', async () => {
    userServiceMock.useValue.getUser = jest.fn().mockResolvedValue({
      ...activeAutomationMock,
      openCondition: {
        field: 'price1',
        operator: '==',
  ...
  it('should NOT start automation (private key)', async () => {
    ...
    await expect(automationController.startAutomation(
      activeAutomationMock.id!,
      authorization,
    )).rejects
      .toEqual('You must have a private key is settings before you update a automation.');
    userServiceMock.useValue.getUser = jest
      .fn()
      .mockResolvedValue({ ...activeAutomationMock });
  });
